...
public class FFindPathWeightFloorValOfk {
	...
	public static void main(String[] args) {
		...
		floorOFWtPath(graph, 0, 5, visited, 0 + "", 0, 42);
		System.out.println(fPathwt);

	}

	private static void floorOFWtPath(ArrayList<Edge>[] graph, int src, int dest, boolean[] visited, String psf, int wsf,
			int val) {
		...
		for(Edge e : graph[src]) {
			if(visited[e.nbr] == false) {
				floorOFWtPath(graph, e.nbr, dest, visited, psf + e.nbr, wsf + e.wt, 42);
			}
		}
		visited[src] = false;
	}
